...
import SpacedRepetitionItem, { ISpacedRepetitionItem } from '../models/SpacedRepetitionItem';
...
export const calculateNextReview = (item: ISpacedRepetitionItem, performance: ReviewPerformance) => {
    let { repetitions, currentInterval, easeFactor, lapses } = item;
    const { quality } = performance;

    if (quality < 3) { // Failed to recall or recalled with significant difficulty
...
export const addSpacedRepetitionItem = async (
    ...
    initialNextReviewDate.setDate(initialNextReviewDate.getDate() + INITIAL_INTERVAL_DAYS);
...
export const getDueReviewItems = async (userId: mongoose.Types.ObjectId): Promise<ISpacedRepetitionItem[]> => {
...
export const getSpacedRepetitionItemById = async (itemId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId): Promise<ISpacedRepetitionItem | null> => {
...
export const updateSpacedRepetitionItemContent = async (
    itemId: mongoose.Types.ObjectId,
    userId: mongoose.Types.ObjectId,
    updates: { originalContent?: string; answerContent?: string; tags?: string[] }
): Promise<ISpacedRepetitionItem | null> => {
    const item = await SpacedRepetitionItem.findOne({ _id: itemId, userId });
    if (!item) {
        throw new Error('Spaced repetition item not found or access denied.');
...
export const deleteSpacedRepetitionItem = async (itemId: mongoose.Types.ObjectId, userId: mongoose.Types.ObjectId): Promise<boolean> => {

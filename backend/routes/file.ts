...
const upload = multerImport({
  ...
  onError: function (err: any, next: express.NextFunction) {
    // Corrected type for next
    console.error('[Multer Error] Error during file processing:', err);
    // Ensure a JSON response is sent if multer errors
    if (err && !err.status) err.status = 500;
    if (err && !err.message)
      err.message = 'Error during file upload processing by multer.';
...
router.post(
  ...
  (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    console.log(
      '[Syllabus OCR Pre-Multer] Request received. Headers:',
      JSON.stringify(req.headers),
    );
    console.log(
      '[Syllabus OCR Pre-Multer] Body (keys):',
      req.body ? Object.keys(req.body) : 'No body yet (expected for multipart)',
    );
  ...
  async (req: any, res: express.Response, next: express.NextFunction) => {
    console.log(
      '[Syllabus OCR Post-Multer] Request received, file processing by multer supposedly complete.',
    );
    try {
      const userId = req.body.userId;
      console.log(
        `[Syllabus OCR] User ID: ${userId}, File received: ${req.file ? req.file.originalname : 'No file'}`,
      );

      if (!req.file || !userId) {
        console.error(
          '[Syllabus OCR] Validation failed: File or userId missing.',
        );
        const err: any = new Error('File and userId are required.');
        err.status = 400;
        return next(err);
      ...
      console.log(
        '[Syllabus OCR] File uploaded to Firebase Storage. URL:',
        fileUrl,
      );
      ...
      if (
        ...
        try {
          ...
          if (req.file.mimetype === 'application/pdf') {
            console.log(
              '[Syllabus OCR] Processing PDF file directly with Tesseract.',
            );
          }
          const result = await Tesseract.recognize(req.file.buffer, 'eng', {});
          ocrText = result.data.text;
          console.log(
            `[Syllabus OCR] Tesseract OCR completed. Text length: ${ocrText.length}`,
          );
        } catch (tesseractError: any) {
          console.error(
            '[Syllabus OCR] Tesseract OCR processing error:',
            tesseractError,
          );
        ...
        if (req.file.mimetype && req.file.mimetype.startsWith('text/')) {
          ...
          console.warn(
            `[Syllabus OCR] Unsupported file type: ${req.file.mimetype}`,
          );
          const err: any = new Error(
            'Unsupported file type for syllabus processing. Please use PDF, image, or text files.',
          );
          err.status = 415;
          return next(err);
        }
      ...
      if (ocrText && ocrText.trim() !== '') {
        try {
          console.log(
            `[Syllabus OCR] Sending OCR text to /api/ai/syllabus/process. Length: ${ocrText.length}`,
          );
          const aiRes = await axios.post(
          ...
          if (aiRes.data && aiRes.data.entities) {
            ...
            console.log(
              '[Syllabus OCR] Successfully extracted entities from AI service.',
            );
          } else {
            console.warn(
              '[Syllabus OCR] AI response missing entities:',
              aiRes.data,
            );
          ...
          if (aiError.response) {
            ...
            console.error(
              '  Headers:',
              JSON.stringify(aiError.response.headers),
            );
        ...
        console.log(
          '[Syllabus OCR] No OCR text generated or text is empty. Skipping AI entity extraction.',
        );
      }
      ...
      await syllabusDocRef.set({
        ...
        analysisStatus: extractedEntities
          ? 'completed'
          : ocrText
            ? 'ocr_done_ner_failed'
            : 'ocr_failed',
      ...
      console.log(
        `[Syllabus OCR] Data saved to Firestore. Analysis ID: ${analysisId}`,
      );
      ...
      console.error(
        '[Syllabus OCR] Overall error in /syllabus-ocr route (before next(error)):',
        error,
      );
      next(error);
    }

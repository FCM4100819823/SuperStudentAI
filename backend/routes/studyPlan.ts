...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!title) {
      return res
        .status(400)
        .json({ message: 'Title is required for a study plan.' });
    }
      ...
      if (syllabusAnalysisId) {
        try {
          const doc = await admin
            .firestore()
            .collection('syllabusAnalyses')
            .doc(syllabusAnalysisId)
            .get();
          if (doc.exists) {
            syllabusTextContent =
              doc.data()?.extractedText || doc.data()?.fullText; // Or however you store it
          }
        } catch (err) {
          console.warn(
            `Could not fetch syllabus content for ID ${syllabusAnalysisId}:`,
            err,
          );
        }
      }
      ...
      if (syllabusTextContent || userGoals) {
        try {
          ...
          const aiApiResponse = await axios.post(
            `http://localhost:${process.env.PORT || 3001}/api/ai/study-plan/generate`,
            {
          ...
          if (aiApiResponse.data && Array.isArray(aiApiResponse.data.tasks)) {
            generatedTasks = [
              ...generatedTasks,
              ...aiApiResponse.data.tasks.map((task: AISuggestedTask) => ({
                ...task,
          ...
          aiSuggestionsFromGeneration =
            'AI task generation was attempted but failed. ' +
            (aiError.response?.data?.message || aiError.message);
        }
      }
    }
    ...
    res
      .status(500)
      .json({ message: 'Failed to create study plan', error: error.message });
  }
});
...
router.post('/:planId/optimize', async (req: Request, res: Response) => {
  try {
    ...
    if (!studyPlan) {
      return res
        .status(404)
        .json({ message: 'Study plan not found or access denied.' });
    }

    if (!studyPlan.tasks || studyPlan.tasks.length === 0) {
      return res
        .status(400)
        .json({ message: 'No tasks in the study plan to optimize.' });
    }

    try {
      console.log(`Calling AI to optimize tasks for study plan ID: ${planId}`);
      const aiApiResponse = await axios.post(
        `http://localhost:${process.env.PORT || 3001}/api/ai/study-plan/optimize`,
        {
    ...
    res
      .status(500)
      .json({ message: 'Failed to optimize study plan', error: error.message });
  }
});
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to fetch study plans', error: error.message });
  }
});
...
router.get('/:planId', async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to fetch study plan', error: error.message });
  }
});
...
router.put('/:planId', async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to update study plan', error: error.message });
  }
});
...
router.delete('/:planId', async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to delete study plan', error: error.message });
  }
});
...
router.put('/:planId/tasks/:taskId', async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: 'Invalid Study Plan or Task ID format.' });
    }
    ...
    if (!task) {
      return res
        .status(404)
        .json({ message: 'Task not found in this study plan.' });
    }
...
router.delete('/:planId/tasks/:taskId', async (req: Request, res: Response) => {
  try {
    ...
    // @ts-ignore
    const taskIndex = studyPlan.tasks.findIndex((task) =>

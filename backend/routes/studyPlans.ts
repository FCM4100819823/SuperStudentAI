...
router.post('/', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Calculate progress
    const completedTasks = savedStudyPlan.tasks.filter(task => task.status === 'completed').length;
...
router.put('/:id', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Calculate progress
    const completedTasks = updatedStudyPlan.tasks.filter(task => task.status === 'completed').length;
...
router.put('/:id/tasks/:taskId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.uid;
    const studyPlanId = req.params.id;
    ...
    const task = studyPlan.tasks.find(t => t._id && t._id.toString() === taskId);
...
router.post('/:id/tasks', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.uid;
    const studyPlanId = req.params.id;
    ...
    const { title, description, dueDate, estimatedHours, relatedTopic } = req.body;
...
router.delete('/:id/tasks/:taskId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.uid;
    const studyPlanId = req.params.id;
    const taskId = req.params.taskId;
    ...
    const taskIndex = studyPlan.tasks.findIndex(t => t._id && t._id.toString() === taskId);

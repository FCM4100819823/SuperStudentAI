...
router.post('/', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    try {
        const { title, description, startDate, endDate, eventType, sourceSyllabusAnalysisId, allDay, location, courseId } = req.body;
        
        const actualUserId = req.user?.uid; // Use uid from Firebase DecodedIdToken

        if (!actualUserId || !title || !startDate || !eventType) {
            return res.status(400).json({ message: 'UserId, title, startDate, and eventType are required.' });
        }
...
router.get('/', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        if (!userId) {
            return res.status(400).json({ message: 'User ID is required to fetch events.' });
        }
        // Ensure find query uses string for userId if your schema defines it as String
        const events = await CalendarEvent.find({ userId: userId }).sort({ startDate: 'asc' }); 
...
router.put('/:eventId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    try {
        const { title, description, startDate, endDate, eventType, allDay, location, courseId } = req.body;
        ...
        eventToUpdate.startDate = startDate ? new Date(startDate) : eventToUpdate.startDate;
        eventToUpdate.endDate = endDate ? new Date(endDate) : eventToUpdate.endDate;
        eventToUpdate.eventType = eventType !== undefined ? eventType : eventToUpdate.eventType;
        eventToUpdate.allDay = typeof allDay === 'boolean' ? allDay : eventToUpdate.allDay;
        eventToUpdate.location = location !== undefined ? location : eventToUpdate.location;
        if (courseId) {
...
router.delete('/:eventId', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    try {
        const eventId = req.params.eventId;

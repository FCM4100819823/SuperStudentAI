...
const CreateStudyPlanScreen = ({ route }) => {
  ...
  const [title, setTitle] = useState(
    syllabusTitle ? `Study Plan for ${syllabusTitle}` : '',
  );
  ...
  const handleCreatePlan = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        Alert.alert(
          'Success',
          'Study plan created successfully! AI is generating tasks if applicable.',
        );
        navigation.goBack(); // Or navigate to the new plan's detail screen
      } else {
        Alert.alert(
          'Error',
          'Failed to create study plan. ' + (response.data?.message || ''),
        );
      }
    } catch (error) {
      console.error(
  ...
  const showDatePicker = (mode) => {
    setIsPickerShow(mode);
    setShowPicker(true);
  };

  const onChange = (event, selectedDate) => {
    const currentDate =
      selectedDate || (isPickerShow === 'startDate' ? startDate : endDate);
    setShowPicker(Platform.OS === 'ios');
    if (isPickerShow === 'startDate') {
      ...
      <ScrollWrapper {...scrollWrapperProps}>
        ...
        {syllabusAnalysisId && (
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>
              This plan can be linked to your uploaded syllabus: '
              {syllabusTitle || 'Selected Syllabus'}'. AI will use it to suggest
              tasks if enabled below.
            </Text>
          </View>
        )}
        ...
        {useAISuggestions && (
          <View style={styles.inputGroup}>
            <Text style={styles.label}>
              Your Goals / Key Topics (Optional, for AI)
            </Text>
            <TextInput
              style={[styles.input, styles.textArea]}
              value={userGoals}
        ...
        {showPicker && (
          <DateTimePicker
            ...
            value={isPickerShow === 'startDate' ? startDate : endDate}
            mode="date"
            is24Hour={true}
            display="default"
            onChange={onChange}
            minimumDate={new Date()} // Optional: Prevent selecting past dates
            // Custom styling for DateTimePicker is limited, but some themes might be picked up by OS
          />
        )}

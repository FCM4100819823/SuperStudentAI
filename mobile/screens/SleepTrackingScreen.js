...
const SleepTrackingScreen = ({ navigation }) => {
  ...
  const handleTimeChange = (event, selectedTime, type) => {
    const currentTime =
      selectedTime || (type === 'sleep' ? sleepTime : wakeTime);
    if (Platform.OS === 'ios') {
  ...
  const handleSubmitEntry = async () => {
    ...
    if (
      wakeTime <= sleepTime &&
      !(wakeTime.getHours() < sleepTime.getHours())
    ) {
    ...
    <View style={styles.sleepEntryItem}>
      <View style={styles.sleepDetails}>
        <Text style={styles.sleepDate}>
          {new Date(item.localSleepDateString).toLocaleDateString()}
        </Text>
        <Text style={styles.sleepInfo}>
          Slept:{' '}
          {new Date(item.sleepTime).toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit',
          })}
        </Text>
        ...
        {item.notes && (
          <Text style={styles.sleepInfo}>Notes: {item.notes}</Text>
        )}
      </View>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.container}>
        ...
        <Text style={styles.headerSubtitle}>
          Log your sleep patterns for better insights.
        </Text>
        ...
        <View style={styles.inputContainer}>
            ...
            {SLEEP_QUALITIES.map((quality) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  sleepQuality === quality && styles.selectedQualityButton,
                ]}
                ...
                <Text
                  style={[
                    ...
                    sleepQuality === quality &&
                      styles.selectedQualityButtonText,
                  ]}
                >
                  {quality}
                </Text>
          ...
          <TouchableOpacity style={styles.button} onPress={handleSubmitEntry}>
            <Text style={styles.buttonText}>
              {editingEntry ? 'Update Entry' : 'Log Sleep'}
            </Text>
          </TouchableOpacity>
          {editingEntry && (
            <TouchableOpacity
        ...
        {loading ? (
          ...
          <Text style={styles.emptyListText}>
            No sleep entries yet. Sweet dreams!
          </Text>
        )}
      </ScrollView>
    </SafeAreaView>

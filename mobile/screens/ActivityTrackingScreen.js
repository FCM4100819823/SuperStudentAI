...
import { View, Text, StyleSheet, TouchableOpacity, FlatList, TextInput, Alert, SafeAreaView, ScrollView, Platform } from 'react-native';
...
const getStyles = (colors) => StyleSheet.create({
  ...
  headerTitle: { fontSize: 26, fontWeight: 'bold', color: colors.primary, marginBottom: 10, textAlign: 'center' },
  ...
  listTitle: { fontSize: 20, fontWeight: 'bold', color: colors.primary, marginTop: 20, marginBottom: 10 },
  activityItem: {
    backgroundColor: colors.card,
    borderRadius: 10,
    padding: 15,
...
const ACTIVITY_TYPES = ['Walking', 'Running', 'Cycling', 'Gym', 'Swimming', 'Yoga', 'Other'];
...
const ActivityTrackingScreen = ({ navigation }) => {
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      const userActivities = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const handleSubmitActivity = async () => {
    ...
    if (!userId) {
      Alert.alert("Not Logged In", "You need to be logged in to save activities.");
      return;
    }
    ...
    try {
      if (editingActivity) {
        const activityRef = doc(firestoreDb, 'userActivities', editingActivity.id);
  ...
  const handleEdit = (activity) => {
    ...
    setDate(activity.date.toDate ? activity.date.toDate() : new Date(activity.localDateString)); 
  };
  ...
  const renderActivityItem = ({ item }) => (
    <View style={styles.activityItem}>
      <View style={styles.activityDetails}>
        ...
        {item.calories && <Text style={styles.activityInfo}>Calories: {item.calories} kcal</Text>}
        <Text style={styles.activityInfo}>Date: {item.date.toDate ? item.date.toDate().toLocaleDateString() : new Date(item.localDateString).toLocaleDateString()}</Text>
      </View>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.container}>
        ...
        <Text style={styles.headerSubtitle}>Log your workouts and stay active.</Text>
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={{flexDirection: 'row', flexWrap: 'wrap', marginBottom: 10}}>
            {ACTIVITY_TYPES.map(type => (
                <TouchableOpacity 
                    ...
                    style={[
                        styles.input, 
                        {marginRight: 5, marginBottom: 5, paddingVertical: 8, paddingHorizontal: 12},
                        activityType === type && {backgroundColor: colors.primary, borderColor: colors.primary}
                    ]} 
                    onPress={() => setActivityType(type)}
                >
                    <Text style={activityType === type && {color: colors.white}}>{type}</Text>
                </TouchableOpacity>
            ))}
          </View>
          ...
          <TouchableOpacity style={styles.button} onPress={handleSubmitActivity}>
            <Text style={styles.buttonText}>{editingActivity ? 'Update Activity' : 'Log Activity'}</Text>
          </TouchableOpacity>
          {editingActivity && (
            <TouchableOpacity style={[styles.button, {backgroundColor: colors.subtext}]} onPress={() => {setEditingActivity(null); setActivityType(ACTIVITY_TYPES[0]); setDuration(''); setCalories(''); setDate(new Date());}}>
        ...
        {loading ? (
          ...
          <Text style={styles.emptyListText}>No activities logged yet. Add one above!</Text>
        )}
      </ScrollView>
    </SafeAreaView>
  );

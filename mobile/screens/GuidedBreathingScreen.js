    ...
    if (isRunning && timeLeft > 0) {
      ...
      const runPhase = (currentPhaseStartTime) => {
        const elapsedInCycle =
          (Date.now() - currentPhaseStartTime) % CYCLE_TIME;

        if (elapsedInCycle < INHALE_TIME) {
          setBreathPhase('Inhale');
          phaseTimeout = setTimeout(
            () => runPhase(currentPhaseStartTime),
            INHALE_TIME - elapsedInCycle,
          );
        } else if (elapsedInCycle < INHALE_TIME + HOLD_TIME) {
          setBreathPhase('Hold');
          phaseTimeout = setTimeout(
            () => runPhase(currentPhaseStartTime),
            INHALE_TIME + HOLD_TIME - elapsedInCycle,
          );
        } else {
          setBreathPhase('Exhale');
          phaseTimeout = setTimeout(
            () => runPhase(currentPhaseStartTime),
            CYCLE_TIME - elapsedInCycle,
          );
        }
      };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        ...
        <Text style={styles.instructions}>
          Follow the rhythm. Inhale as the circle expands, hold, and exhale as
          it contracts.
        </Text>
        ...
        <View style={styles.controlsContainer}>
          <TouchableOpacity
            style={[
              ...
              isRunning && timeLeft === 0 && styles.disabledButton,
            ]}
            onPress={toggleTimer}

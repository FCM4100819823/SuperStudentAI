...
const WellbeingScreen = ({ navigation }) => {
      ...
      if (auth.currentUser) {
        try {
          ...
          if (!querySnapshot.empty) {
            ...
            setLastMoodEntry({
              ...lastEntry,
              timestamp: lastEntry.timestamp?.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' ' + lastEntry.timestamp?.toDate().toLocaleDateString()
            });
          }
        } catch (error) {
  ...
  const submitMood = async () => {
    ...
    try {
      ...
      Alert.alert("Mood Logged", `You've logged your mood as ${selectedMood.name}.`);
      setLastMoodEntry({ 
        ...moodEntry, 
  ...
  const FeatureCard = ({ title, icon, children, onPress }) => (
    <TouchableOpacity style={styles.card} onPress={onPress} disabled={!onPress}>
      <View style={styles.cardHeaderContainer}>
        {icon && <Ionicons name={icon} size={24} color={colors.primary} style={styles.cardIcon} />}
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.screenContainer} contentContainerStyle={styles.scrollContentContainer}>
        ...
        <FeatureCard title="How are you feeling?" icon="happy-outline">
          <View style={styles.moodSelectorContainer}>
            {moods.map((mood) => (
              <TouchableOpacity 
                ...
                style={[
                  styles.moodButton,
                  selectedMood?.name === mood.name && { backgroundColor: mood.color, borderColor: mood.color }, // Keep dynamic mood color for selection
              ...
              >
                ...
                />
                <Text style={[styles.moodText, selectedMood?.name === mood.name && {color: '#FFF'}]}>{mood.name}</Text>
              </TouchableOpacity>
            ))}
          </View>
          <TouchableOpacity 
          ...
          {loadingMood ? <ActivityIndicator style={{marginTop: 10}} color={colors.primary} /> :
            lastMoodEntry && (
        ...
        <FeatureCard title="Journal Prompt" icon="pencil-outline">
          ...
          <TouchableOpacity style={styles.secondaryButton} onPress={() => Alert.alert("Journal", "Journaling feature coming soon!")}>
            <Text style={styles.secondaryButtonText}>Write in Journal</Text>
          </TouchableOpacity>
        </FeatureCard>

        <FeatureCard title="Quick Mindfulness" icon="leaf-outline">
          <TouchableOpacity style={styles.mindfulnessItem} onPress={() => Alert.alert("Breathing Exercise", "Feature coming soon!")}>
            <Ionicons name="pulse-outline" size={22} color={colors.success} />
            <Text style={styles.mindfulnessText}>Guided Breathing (2 min)</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.mindfulnessItem} onPress={() => Alert.alert("Meditation", "Feature coming soon!")}>
        ...
        <FeatureCard title="Positive Affirmation" icon="sparkles-outline">
            <Text style={styles.affirmationText}>"I am capable and I will succeed today."</Text>
        </FeatureCard>
